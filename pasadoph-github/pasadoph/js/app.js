...
(function () {
  ...
  async function initAuth() {
    ...
    supa.auth.onAuthStateChange(function (_evt, session) {
      ...
      state.user = session ? session.user : null;
      if (state.user && !wasIn) {
        if (state.inRecovery) { go("resetpass"); return; }
  ...
  function renderTopbar() {
    if (state.user) {
      $actions.innerHTML =
        ...
        '<button class="btn btn-sm tb-btn" id="tbDash">My&nbsp;review</button>' +
        '<button class="btn btn-sm tb-btn tb-out" id="tbOut">Log&nbsp;out</button>';
  ...
  function sessionStorageSafeSet(k, v) { try { window.sessionStorage.setItem(k, v); } catch (e) {} }
  function sessionStorageSafeGet(k) { try { return window.sessionStorage.getItem(k); } catch (e) { return null; } }
  function sessionStorageSafeDel(k) { try { window.sessionStorage.removeItem(k); } catch (e) {} }
    ...
    document.getElementById("afGo").onclick = async function () {
      ...
      try {
        ...
        if (isReg && out.data && out.data.user && !out.data.session) {
          go("verify", { email: email, pass: pass });
          return;
        }
  ...
  function renderVerify(payload) {
    var email = typeof payload === "string" ? payload : (payload && payload.email);
    var passMem = payload && typeof payload === "object" ? payload.pass : null;
    $app.innerHTML =
      '<div class="auth-card" style="text-align:center">' +
        ...
        '<p style="font-size:0.85rem;color:var(--muted);margin-bottom:10px">Click the link in the email to activate your account.</p>' +
        '<div class="form-msg ok" style="text-align:left;margin-bottom:14px"><b>Can\'t find it?</b> Check your <b>Spam</b> or Promotions folder \u2014 then tap <b>\u201cReport not spam\u201d</b> so future PasadoPH emails land in your inbox.</div>' +
        '<p style="font-size:0.82rem;color:var(--muted);margin-bottom:14px" id="vAuto">Once you click the link \u2014 on any device \u2014 this page will log you in automatically.</p>' +
    ...
    // Cross-device sync: if they verify on their phone, this tab logs in by itself.
    if (passMem && supa) {
      var vTries = 0;
      var vTimer = setInterval(async function () {
        vTries++;
        if (state.view !== "verify" || vTries > 60) { clearInterval(vTimer); return; }
        try {
          var r = await supa.auth.signInWithPassword({ email: email, password: passMem });
          if (r && r.data && r.data.session) { clearInterval(vTimer); /* onAuthStateChange routes to dashboard */ }
  ...
  var TT_CONTENT = { content_id: "pasadoph_lifetime", content_name: "PasadoPH Lifetime Access", content_type: "product", currency: "PHP", value: 299 };
  var _ttFired = {};

  async function ttSha256(str) {
    try {
      if (!str || !window.crypto || !window.crypto.subtle) return null;
      var norm = String(str).trim().toLowerCase();
      var buf = await window.crypto.subtle.digest("SHA-256", new TextEncoder().encode(norm));
      return Array.prototype.map.call(new Uint8Array(buf), function (b) { return ("0" + b.toString(16)).slice(-2); }).join("");
    } catch (e) { return null; }
  }

  async function ttIdentify() {
    // Only send email/external_id when genuinely available; both SHA-256 hashed. No phone (not collected).
    if (typeof window.ttq === "undefined" || !state.user || state.user.preview) return;
    var ident = {};
    if (state.user.email) { var e = await ttSha256(state.user.email); if (e) ident.email = e; }
    if (state.user.id) { var x = await ttSha256(state.user.id); if (x) ident.external_id = x; }
    if (Object.keys(ident).length) { try { window.ttq.identify(ident); } catch (e) {} }
  }

  async function ttTrack(eventName, dedupeKey) {
    if (typeof window.ttq === "undefined") return;
    if (dedupeKey) { if (_ttFired[dedupeKey]) return; _ttFired[dedupeKey] = true; }
    await ttIdentify();
    try {
      window.ttq.track(eventName, {
        content_id: TT_CONTENT.content_id,
        content_name: TT_CONTENT.content_name,
        content_type: TT_CONTENT.content_type,
        currency: TT_CONTENT.currency,
        value: TT_CONTENT.value
      });
    } catch (e) {}
  }

  function renderUpgrade() {
    ttTrack("ViewContent", "viewcontent_upgrade");
    $app.innerHTML =
    ...
    document.getElementById("fSend").onclick = async function () {
      ...
      try {
        ...
        $msg.innerHTML = out.error
          ...
          : '<div class="form-msg ok">If an account exists for that email, a reset link is on its way. Check your inbox and Spam folder.</div>';
      } catch (e) {
        $msg.innerHTML = '<div class="form-msg err">' + esc(friendlyAuthError(e)) + "</div>";
      }
      this.disabled = false;
  ...
  (async function boot() {
    ...
    if (h.indexOf("type=recovery") >= 0 || qs.indexOf("type=recovery") >= 0) {
      state.inRecovery = true;
      try { sessionStorageSafeSet("pasado_recovery", "1"); } catch (e) {}
    }
    if (!state.inRecovery && sessionStorageSafeGet("pasado_recovery") === "1") {
      state.inRecovery = true;
    }
    var linkErr = h.indexOf("error_code=otp_expired") >= 0 || h.indexOf("error=access_denied") >= 0;
    if (linkErr) {
      state.flashMsg = "That link has expired or was already used. Log in below \u2014 or request a fresh link if you still need one.";
      try { window.history.replaceState({}, "", window.location.pathname + window.location.search); } catch (e) {}
    }
    await initAuth();
    renderTopbar();
    if (linkErr && !state.user) { go("auth", "login"); return; }
    if (state.inRecovery) {
